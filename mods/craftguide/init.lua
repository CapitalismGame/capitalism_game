...
function craftguide:get_init_items()
	...
	local function in_mtg(a)
		local modname = a:match("^([a-z_]+)%:")
		local path    = minetest.get_modpath(modname)
		return not path or path:match("/mtg/")
	end

	sort(items_list, function(a, b)
		local a_mtg = in_mtg(a)
		local b_mtg = in_mtg(b)
		return (not a_mtg and (b_mtg or a < b)) or (b_mtg and a < b)
	end)
